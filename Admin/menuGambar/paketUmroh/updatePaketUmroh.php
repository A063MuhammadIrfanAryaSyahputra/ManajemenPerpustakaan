...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE umroh SET nama = '$name', deskripsi = '$deskripsi', itenary1 = '$itenary1', itenary2 = '$itenary2', itenary3 = '$itenary3', itenary4 = '$itenary4' WHERE id = $id";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if (isset($_FILES['new_second_image']) && $_FILES['new_second_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($secondImageTmpPath, $secondDestPath)) {
                $sql = "UPDATE umroh SET cover = '$secondImageName' WHERE id = $id";
                $conn->query($sql);
            }
        }

        header("Location: paketUmroh.php");
    } else {
